...
class Effect:
    ...
    def from_bytes(cls, data: bytes) -> 'Effect':
        ...
        return cls(id, raw_amount, unknown1, affinity_level, affinity_type, unknown2)
...
class Item:
    ...
    def from_process(cls, process: pymem.Pymem, index: int) -> 'Item':
        address = (process.base_address + Item.ITEMS_START + index * Item.STRUCT_SIZE)
        data = process.read_bytes(address, Item.STRUCT_SIZE)
        return cls.from_bytes(data, address=address, process=process)
            
    @classmethod
    def from_bytes(cls, data: bytes, address: int = 0, process: Optional[pymem.Pymem] = None):
        ...
        return Item(process, address, data, id[0], amount, level, original_level,
                    rarity, status, slot_pos, effects,
                    attack, defense, magic, resist,
                    job1, job2, skills, summon)
    ...
    def hex(self) -> str:
        return '\n'.join(textwrap.wrap(' '.join(textwrap.wrap(self._buffer.hex(), 2)), 48))
    ...
    def should_keep(self) -> bool:
        ...
        for effect in self.effects:
            if filters.getint(effect.name, fallback=-1) < effect.affinity_level:
                return True
        
        slot_type = db_entry.slots
        if slot_type != 'Accessory':
            try:
                if Config['Keep Artifacts'].getboolean(slot_type, fallback=False):
                    if self.job1[0] != 0 and self.job2[0] != 0:
                        return True
                        
            except ValueError:
                if Config['Keep Artifacts'].get(slot_type, fallback='') == 'blessed':
                    if self.summon[0] != 0:
                        return True
            
            min_affinity = Config['Minimum Affinity'].getint(slot_type, fallback=9999)
            ...
            for skill in item.skills:
                if skill != 0:
                    ...
                    if 'Weapon' in item_type:
                        ...
                        raise Exception(f'unexpected skill {skill} on {item.name}')
                        
        for item in not_kept:
            for skill in item.skills:
                if skill != 0:
                    if Config['Skills'].getboolean('Keep One Of Each Weapon Skill'):
                        if weapon_skills.get(skill) is False:
                            results.append(item)
                            weapon_skills[skill] = True
                    if Config['Skills'].getboolean('Keep One Of Each Accessory Skill'):
    ...
    def from_file(cls, filename: Path):
        ...
        with filename.open('rb') as f:
            ...
            for item in range(size):
                item = Item.from_bytes(buffer[index : index + Item.STRUCT_SIZE], address=index)
                items.append(item)
